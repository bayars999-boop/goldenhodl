...
type Country = { name: string; code: string; dial: string; timezones: string[] };
...
const timezoneHints: Record<string, string[]> = { MN: ['Asia/Ulaanbaatar', 'Asia/Hovd', 'Asia/Choibalsan'], US: ['America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles'], DE: ['Europe/Berlin'], JP: ['Asia/Tokyo'], BR: ['America/Sao_Paulo'], ZA: ['Africa/Johannesburg'] };
const regionNames = new Intl.DisplayNames(['en'], { type: 'region' });
const countries: Country[] = countryCodes
  .map((code) => ({ name: regionNames.of(code) || code, code, dial: dialCodes[code] || '', timezones: timezoneHints[code] || [] }))
...
export default function CopyTradingPage() {
  ...
  const selectedCountry = countries.find((item) => item.name === country) || countries[0];
  ...
  useEffect(() => {
    const detected = countries.find((item) => item.timezones.includes(Intl.DateTimeFormat().resolvedOptions().timeZone));
    if (detected) setCountry(detected.name);
  }, []);
  ...
  const startVerification = () => {
    ...
    if (!riskAccepted) {
      setError('Please read and accept the risk disclosure.');
      return;
    }
    setError('');
    setStep('captcha');
  ...
  const verifyCode = () => {
    ...
    window.localStorage.setItem('goldmaster-profile', JSON.stringify({
      country: selectedCountry.name, countryCode: selectedCountry.code, dialCode: selectedCountry.dial,
      referralCode: normalizedReferral || null, referralBonus: normalizedReferral ? 25 : 0,
      referralStatus: normalizedReferral ? 'pending' : 'none',
    }));
  ...
  return (
    <main suppressHydrationWarning style={{ minHeight: '100vh', background: colors.page, color: colors.text, fontFamily: 'Arial, sans-serif', padding: '15px 15px 90px' }}>
        ...
        <div style={{ background: colors.surface, border: `1px solid ${colors.border}`, borderRadius: '6px', padding: '24px' }}>
          {step === 'register' && <div>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <select aria-label="Country of residence" value={country} onChange={(event) => setCountry(event.target.value)} style={{ ...inputStyle, flex: 1 }}>
                {countries.map((item) => <option key={item.code}>{item.name}</option>)}
              </select>
              <span style={{ ...inputStyle, width: '76px', color: colors.muted, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '8px' }}><CountryFlag code={selectedCountry.code} size={20} /></span>
            </div>
            <small style={{ color: colors.muted, display: 'block', marginBottom: '12px' }}>Detected phone code: {selectedCountry.dial}</small>
            <div style={{ display: 'flex', gap: '18px', marginBottom: '12px' }}>{(['Email', 'Mobile'] as const).map((item) => <button key={item} type="button" onClick={() => setMode(item)} style={{ background: 'none', border: 0, borderBottom: mode === item ? '2px solid #16a34a' : '2px solid transparent', color: mode === item ? '#0284c7' : colors.text, padding: '8px', fontWeight: 700 }}>{item}</button>)}</div>
            <input aria-label={mode === 'Email' ? 'Email address' : 'Mobile number'} placeholder={mode === 'Email' ? 'Email' : `${selectedCountry.dial} Mobile number`} type={mode === 'Email' ? 'email' : 'tel'} value={contact} onChange={(event) => setContact(event.target.value)} style={inputStyle} />
            ...
            <button type="button" onClick={() => setStep('risk')} style={{ width: '100%', border: `1px solid ${riskAccepted ? '#16a34a' : colors.border}`, borderRadius: '6px', padding: '11px', background: colors.surface, color: riskAccepted ? '#16a34a' : colors.text, fontWeight: 700 }}>Risk disclosure {riskAccepted ? '✓' : 'Read'}</button>
            <button type="button" onClick={startVerification} disabled={!riskAccepted} style={{ width: '100%', marginTop: '12px', border: 0, borderRadius: '6px', padding: '13px', background: riskAccepted ? '#16a34a' : '#e2e8f0', color: riskAccepted ? '#fff' : '#94a3b8', fontWeight: 700, cursor: riskAccepted ? 'pointer' : 'not-allowed' }}>Register</button>
          </div>}
      ...
      <footer style={{ position: 'fixed', bottom: 0, left: 0, right: 0, padding: '16px', background: colors.surface, borderTop: `1px solid ${colors.border}`, color: colors.muted, fontSize: '13px' }}>Risk Warning: Your capital is at risk. Leveraged products may not be suitable for everyone.</footer>
