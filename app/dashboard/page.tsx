...
type Tab = 'Overview' | 'Copy trading' | 'EA rental' | 'Products' | 'Activity' | 'Account' | 'Legal Documents';
...
export default function UserDashboardPage() {
  ...
  const [checkoutPreview, setCheckoutPreview] = useState<{ url: string; currency: string; subtotal: string; tax: string; total: string } | null>(null);
  const [activationMessage, setActivationMessage] = useState('');
  const [lemonReady, setLemonReady] = useState(false);
  const [checkoutLoading, setCheckoutLoading] = useState(false);
  const [selectedLegalDocument, setSelectedLegalDocument] = useState<LegalDocumentId>('privacy');

  const lemonCheckoutUrls: Partial<Record<ServiceKind, string>> = {
    'Copy trading': process.env.NEXT_PUBLIC_LEMON_CHECKOUT_URL_COPY_TRADING,
    'EA rental': process.env.NEXT_PUBLIC_LEMON_CHECKOUT_URL_EA_RENTAL,
  };
  ...
  useEffect(() => {
    ...
    void fetch('/api/profile', { credentials: 'include' }).then(async (response) => {
      const result = await response.json() as { profile?: { full_name?: string; phone?: string; country?: string }; error?: string };
      if (!response.ok) throw new Error(result.error || 'Unable to load profile.');
      setProfile((current) => ({ ...current, fullName: result.profile?.full_name || '', phone: result.profile?.phone || '', country: result.profile?.country || '' }));
    }).catch((error: unknown) => setProfileError(error instanceof Error ? error.message : 'Unable to load profile.'));
  }, [router]);
  ...
  useEffect(() => {
    ...
    if (window.LemonSqueezy) {
      ...
      window.setTimeout(() => setLemonReady(true), 0);
      return;
    }
    ...
    script.onload = () => {
      window.LemonSqueezy?.Setup();
      setLemonReady(Boolean(window.LemonSqueezy));
    };
    script.onerror = () => setLemonReady(false);
  ...
  const requestContactOtp = async () => {
    ...
    setOtpSending(true);
    try {
      const response = await fetch('/api/auth/otp/request', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contact, channel: 'email', purpose: 'contact-verification' }) });
      const result = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) {
        setOtpError(result.error || 'Unable to send the verification code.');
        return;
      }
      setOtpStep('sent');
    } finally {
  ...
  const verifyContactOtp = async () => {
    setOtpError('');
    const response = await fetch('/api/auth/otp/verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contact, purpose: 'contact-verification', code: otpCode }) });
    const result = await response.json().catch(() => ({})) as { verified?: boolean; error?: string };
    if (!response.ok || !result.verified) {
      setOtpError(result.error || 'Incorrect verification code.');
      return;
    }
    setContactVerified(true);
    setOtpStep('idle');
    setOtpCode('');
    setVerificationStep((step) => Math.max(step, 2));
  };
  const saveProfile = async () => {
    setProfileError('');
    const response = await fetch('/api/profile', { method: 'PATCH', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(profile) });
    const result = await response.json() as { profile?: { full_name?: string; phone?: string; country?: string }; error?: string };
    if (!response.ok) {
  ...
  const exportData = async (format: 'json' | 'csv') => {
    setExportError('');
    const response = await fetch(`/api/data-export?format=${format}`, { credentials: 'include' });
    if (!response.ok) {
      const result = await response.json().catch(() => ({})) as { error?: string };
      setExportError(result.error || 'Unable to export your data.');
      return;
    }
  ...
  const openPayment = async () => {
    if (checkoutLoading || !productSetup || !productSetup.balance || Number(productSetup.balance) < (productSetup.kind === 'EA rental' ? 1000 : 500) || !productSetup.duration || !productSetup.agreed) return;

    setCheckoutLoading(true);
    setCheckoutPreview(null);
    setActivationMessage('Calculating price, VAT/tax, and total...');
    try {
      const response = await fetch('/api/lemon-checkout', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ kind: productSetup.kind, balance: Number(productSetup.balance), duration: productSetup.duration, email: contact, documentVersion: '2026-09', acceptanceTimestamp: new Date().toISOString() }),
      });
      const result = await response.json() as { url?: string; amount?: number; currency?: string; subtotal?: string; tax?: string; total?: string; error?: string };
      if (!response.ok || !result.url) throw new Error(result.error || 'Unable to create checkout.');

      // Show the subtotal/VAT/total breakdown and require explicit confirmation before opening the hosted checkout.
      setCheckoutPreview({ url: result.url, currency: result.currency || 'USD', subtotal: result.subtotal || formatMoney(result.amount), tax: result.tax || formatMoney(0), total: result.total || formatMoney(result.amount) });
      setActivationMessage('');
    } catch (error) {
      const fallbackUrl = lemonCheckoutUrls[productSetup.kind];
      if (fallbackUrl && lemonReady && window.LemonSqueezy) {
        window.LemonSqueezy.Url.Open(fallbackUrl);
        setActivationMessage('Secure checkout opened. Configure the backend for dynamic pricing.');
      } else {
        setActivationMessage(error instanceof Error ? error.message : 'Unable to create checkout.');
        setPaymentOpen(true);
      }
    } finally {
  ...
  const confirmSecurePayment = () => {
    if (!checkoutPreview) return;
    setActivationMessage('Redirecting to Lemon Squeezy...');
    if (lemonReady && window.LemonSqueezy) window.LemonSqueezy.Url.Open(checkoutPreview.url);
    else window.location.assign(checkoutPreview.url);
  };
  ...
  const setupReady = productSetup !== null && Number.isFinite(Number(productSetup.balance)) && Number(productSetup.balance) >= (productSetup.kind === 'EA rental' ? 1000 : 500) && Boolean(productSetup.duration) && productSetup.agreed;
  ...
  const renderLegalDocuments = () => {
    const selected = legalDocuments.find((document) => document.id === selectedLegalDocument) || legalDocuments[0];
        return <section style={{ display: 'grid', gap: '14px' }}>
      <section style={card}><p style={{ margin: 0, color: colors.blue, fontSize: '12px', fontWeight: 700, letterSpacing: '1px' }}>ACCOUNT CENTER</p><h2 style={{ margin: '6px 0' }}>Legal Documents</h2><p style={{ color: colors.muted, margin: 0 }}>Review the policies, agreements, and disclosures connected to your account.</p></section>
      <section className="dashboard-legal-workspace" style={{ display: 'grid', gridTemplateColumns: 'minmax(210px, 280px) minmax(0, 1fr)', gap: '14px', alignItems: 'start' }}>
        <aside style={{ ...card, display: 'grid', gap: '8px' }}><small style={{ color: colors.muted, fontWeight: 700, letterSpacing: '1px' }}>DOCUMENT FOLDER</small>{legalDocuments.map((document) => <button type="button" key={document.id} onClick={() => setSelectedLegalDocument(document.id)} style={{ textAlign: 'left', padding: '12px', border: `1px solid ${selectedLegalDocument === document.id ? colors.blue : colors.border}`, borderRadius: '6px', background: selectedLegalDocument === document.id ? colors.surfaceAlt : colors.surface, color: colors.text, cursor: 'pointer' }}><strong style={{ display: 'block' }}>{document.title}</strong><small style={{ display: 'block', color: colors.muted, marginTop: '4px' }}>{document.id === 'terms' || document.id === 'risk' ? 'Agreement / disclosure' : 'Policy'} · Published</small></button>)}</aside>
        <article style={{ ...card, minWidth: 0 }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', alignItems: 'flex-start', flexWrap: 'wrap' }}><div><small style={{ color: colors.blue, fontWeight: 700, letterSpacing: '1px' }}>CURRENT DOCUMENT</small><h2 style={{ margin: '6px 0' }}>{selected.title}</h2><p style={{ color: colors.muted, margin: 0 }}>{selected.summary}</p></div><span style={{ color: '#16a34a', fontWeight: 700, fontSize: '13px' }}>Published · Sep 2026</span></div><div className="dashboard-legal-body" style={{ marginTop: '20px' }}>{selected.sections.map((section) => <section key={section.heading}><h3>{section.heading}</h3>{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.table && <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', minWidth: '760px', borderCollapse: 'collapse' }}><thead><tr>{section.table.headers.map((header) => <th key={header} style={{ padding: '10px', border: `1px solid ${colors.border}`, background: colors.surfaceAlt, textAlign: 'left' }}>{header}</th>)}</tr></thead><tbody>{section.table.rows.map((row) => <tr key={row[0]}>{row.map((cell, index) => <td key={`${row[0]}-${index}`} style={{ padding: '10px', border: `1px solid ${colors.border}`, verticalAlign: 'top' }}>{cell}</td>)}</tr>)}</tbody></table></div>}</section>)}</div><div style={{ padding: '12px', marginTop: '18px', borderRadius: '6px', background: colors.surfaceAlt, color: colors.muted, fontSize: '13px' }}>Questions or correction requests: info@goldenhodl.com</div></article>
      </section>
    </section>;
  };
  ...
  const renderProductCatalog = () => (
    <section style={card}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '16px', flexWrap: 'wrap' }}>
        <div>
          <p style={{ margin: 0, color: colors.blue, fontSize: '12px', fontWeight: 700, letterSpacing: '1px' }}>DIGITAL PRODUCTS</p>
          <h2 style={{ margin: '6px 0' }}>Choose a product</h2>
          <p style={{ margin: 0, color: colors.muted }}>Review the service details, set your account size, and continue to secure checkout.</p>
        </div>
        <span style={{ color: lemonReady ? '#16a34a' : colors.muted, fontWeight: 700, fontSize: '13px' }}>{lemonReady ? 'LEMON CHECKOUT READY' : 'CHECKOUT CONFIGURATION REQUIRED'}</span>
      </div>
      <div className="product-catalog-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '12px', marginTop: '18px' }}>
        {(Object.keys(serviceProducts) as ServiceKind[]).map((kind) => {
          const product = serviceProducts[kind][0];
          const accent = kind === 'Copy trading' ? colors.blue : '#16a34a';
          const configured = Boolean(lemonCheckoutUrls[kind]);
          return <article key={kind} style={{ border: `1px solid ${colors.border}`, borderRadius: '8px', padding: '16px', background: colors.surfaceAlt }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}><h3 style={{ margin: 0 }}>{product.name}</h3><span style={{ color: accent, fontWeight: 700, fontSize: '12px' }}>{kind}</span></div>
            <p style={{ color: colors.muted, lineHeight: 1.5, minHeight: '48px' }}>{product.description}</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', color: colors.muted, fontSize: '13px', marginBottom: '14px' }}><span>Secure checkout</span><span>·</span><span>{kind === 'EA rental' ? '$1,000 minimum' : '$500 minimum'}</span></div>
            <button type="button" onClick={() => beginProductSetup(kind)} style={{ ...button(true), width: '100%', background: accent, borderColor: accent }}>{configured ? 'View details & buy' : 'View details & configure'}</button>
          </article>;
        })}
      </div>
    </section>
    ...
    if (kind === 'Copy trading') {
      ...
      return <section style={{ display: 'grid', gap: '14px' }}>
        ...
        <section style={card}>
          ...
          <nav style={{ display: 'flex', gap: '7px', overflowX: 'auto', margin: '18px 0 0', borderBottom: `1px solid ${colors.border}` }}>{subTabs.map((tab) => <button type="button" key={tab} className={serviceSection === tab ? 'active-nav' : undefined} onClick={() => setServiceSection(tab)} style={{ background: 'transparent', border: 0, borderBottom: serviceSection === tab ? `2px solid ${accent}` : '2px solid transparent', color: serviceSection === tab ? accent : colors.text, padding: '9px 12px', cursor: 'pointer' }}>{tab}</button>)}</nav>
        </section>
    ...
    <main className="dashboard-page" style={{ minHeight: '100vh', background: colors.page, color: colors.text, padding: '62px 20px 40px', fontFamily: 'Arial, sans-serif' }}>
      <section className="dashboard-container" style={{ maxWidth: '1240px', margin: '0 auto' }}>
        <header className="dashboard-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '20px', marginBottom: '22px', flexWrap: 'wrap' }}>
          ...
          <div className="dashboard-actions" style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}><button type="button" onClick={() => exportData('json')} style={button()}>Download JSON</button><button type="button" onClick={() => exportData('csv')} style={button()}>Download CSV</button><button type="button" onClick={() => router.push('/')} style={button()}>Public signal</button><button type="button" onClick={logout} style={button()}>Log out</button></div>
        </header>
        {exportError && <p role="alert" style={{ color: '#dc2626', marginTop: '-14px', marginBottom: '14px' }}>{exportError}</p>}

        <nav className="dashboard-tabs" style={{ display: 'flex', gap: '7px', overflowX: 'auto', marginBottom: '18px', paddingBottom: '3px' }}>{tabs.map((tab) => <button type="button" key={tab} className={activeTab === tab ? 'dark-active-nav' : undefined} onClick={() => setActiveTab(tab)} style={button(activeTab === tab)}>{tab}</button>)}</nav>

        {activeTab === 'Overview' && <div>
          <div className="dashboard-metrics" style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: '12px', marginBottom: '14px' }}>
          ...
          <div className="dashboard-overview-grid" style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: '14px' }}>
          ...
          <div className="dashboard-secondary-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginTop: '14px' }}><section style={card}><h2 style={{ margin: 0, fontSize: '18px' }}>Active services</h2><p style={{ color: colors.muted }}>Your current products and subscriptions</p><div style={{ display: 'grid', gap: '10px' }}>{(Object.keys(serviceProducts) as ServiceKind[]).filter((kind) => selectedServices[kind]).map((kind) => <div key={kind} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', background: colors.surfaceAlt, borderRadius: '6px' }}><span>{selectedServices[kind]}<small style={{ display: 'block', color: colors.muted, marginTop: '4px' }}>{kind} · {serviceData[kind]?.balance === undefined ? 'Setup pending' : 'Active'}</small></span><strong style={{ color: '#16a34a' }}>●</strong></div>)}{!Object.values(selectedServices).some(Boolean) && <p style={{ color: colors.muted }}>No products selected yet.</p>}</div></section><section style={card}><h2 style={{ margin: 0, fontSize: '18px' }}>Recent activity</h2><p style={{ color: colors.muted }}>Your account events</p><p style={{ color: colors.muted }}>{lastUpdated ? lastUpdated.toLocaleString() : '—'} · Data refreshed</p></section></div>
        </div>}

        {activeTab === 'Account' && <section style={{ display: 'grid', gap: '14px' }}>
          <nav aria-label="Account sections" style={{ display: 'flex', gap: '8px', borderBottom: `1px solid ${colors.border}`, paddingBottom: '10px' }}><button type="button" className="dark-active-nav" onClick={() => setActiveTab('Account')} style={button(true)}>Account</button><button type="button" onClick={() => setActiveTab('Legal Documents')} style={button()}>Legal Documents</button></nav>
          <section style={card}><h2 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>Profile <CountryFlag code={(profile as typeof profile & { countryCode?: string }).countryCode} size={22} /></h2><p style={{ color: colors.muted }}>Keep your personal and contact details current. Sensitive identity documents must be stored and verified by a secure backend.</p><div className="dashboard-form-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '12px' }}>{[['Full legal name', 'fullName', 'Your name as shown on ID'], ['Phone number', 'phone', '+976...'], ['Country / region', 'country', 'Mongolia']].map(([label, key, placeholder]) => <label key={key} style={{ color: colors.muted, fontSize: '13px' }}>{label}<input value={profile[key as keyof typeof profile]} placeholder={placeholder} onChange={(event) => setProfile({ ...profile, [key]: event.target.value })} style={{ display: 'block', width: '100%', marginTop: '6px', padding: '10px', border: `1px solid ${colors.border}`, borderRadius: '5px', background: colors.surfaceAlt, color: colors.text }} /></label>)}<label style={{ color: colors.muted, fontSize: '13px' }}>Email<input value={contact} readOnly style={{ display: 'block', width: '100%', marginTop: '6px', padding: '10px', border: `1px solid ${colors.border}`, borderRadius: '5px', background: colors.surfaceAlt, color: colors.muted }} /></label></div><button type="button" onClick={saveProfile} style={{ ...button(true), marginTop: '14px' }}>{profileSaved ? 'Saved securely' : 'Save profile'}</button>{profileError && <p role="alert" style={{ color: '#dc2626', marginTop: '10px' }}>{profileError}</p>}</section>
          <section style={card}><h2 style={{ margin: 0 }}>Verification / KYC</h2><p style={{ color: colors.muted }}>Complete identity, contact, and suitability checks before withdrawals or regulated services are enabled.</p><div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', borderBottom: `1px solid ${colors.border}` }}><strong style={{ color: contactVerified ? '#16a34a' : colors.muted }}>{contactVerified ? '✓' : 1}</strong><div style={{ flex: 1 }}><b>Contact verification</b><small style={{ display: 'block', color: colors.muted, marginTop: '3px' }}>Email OTP sent to {contact || 'your account email'}</small>{otpStep === 'sent' && !contactVerified && <div style={{ marginTop: '8px', display: 'flex', gap: '8px', alignItems: 'center' }}><input aria-label="Verification code" inputMode="numeric" maxLength={6} value={otpCode} onChange={(event) => setOtpCode(event.target.value.replace(/\D/g, ''))} style={{ width: '110px', padding: '8px', border: `1px solid ${colors.border}`, borderRadius: '4px', background: colors.surfaceAlt, color: colors.text }} /><button type="button" onClick={verifyContactOtp} disabled={otpCode.length !== 6} style={button(true)}>Verify</button></div>}{otpError && <small style={{ display: 'block', color: '#dc2626', marginTop: '6px' }}>{otpError}</small>}</div>{!contactVerified && <button type="button" onClick={requestContactOtp} disabled={otpSending} style={button(true)}>{otpSending ? 'Sending...' : otpStep === 'sent' ? 'Resend code' : 'Send code'}</button>}</div>{[['Identity verification', 'Government ID and liveness check', verificationStep >= 2], ['Address verification', 'Proof of address', verificationStep >= 3], ['Suitability review', 'Risk questionnaire and terms', verificationStep >= 4]].map(([label, detail, done], index) => <div key={String(label)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 0', borderBottom: `1px solid ${colors.border}` }}><strong style={{ color: done ? '#16a34a' : colors.muted }}>{done ? '✓' : index + 2}</strong><div style={{ flex: 1 }}><b>{label}</b><small style={{ display: 'block', color: colors.muted, marginTop: '3px' }}>{detail}</small></div>{!done && index === verificationStep - 2 && <button type="button" onClick={() => setVerificationStep((step) => Math.min(4, step + 1))} style={button(true)}>Continue</button>}</div>)}</section>
        ...
        {activeTab === 'Legal Documents' && renderLegalDocuments()}
        {(activeTab === 'Copy trading' || activeTab === 'EA rental') && renderServiceWorkspace(activeTab)}
        {activeTab === 'Products' && <><div style={{ marginBottom: '14px' }}>{renderProductCatalog()}</div>{renderProductRecords()}</>}
